 ...
 *
 * <p>카운트는 ⑦ 의 Micrometer 카운터({@code drawe.output.hallucinated_citation}, {@code
 * drawe.output.citation_removed})로 박제될 입력이다. DoD: {@code hallucinatedCount == 0}.
 *
 ...
 * @param noRefs references 가 비어 있었는지(§5.1-5). 이때의 환각은 메트릭에서 {@code source=no_refs} 로 분류한다(범위 위반
 *     citations_field/body_scan 과 구분).
 */
public record IntegrityResult(
    ComposedOutput output, int hallucinatedCitations, int hallucinatedBodyTokens, boolean noRefs) {
