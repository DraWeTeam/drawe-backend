 ...
 *
 * <p>S2' Phase 6 — Redis 기반 멀티턴 효율 캐시. 24h TTL 자동 만료, cache miss 시 MySQL
 * ({@code LlmMessage.references_json}) 에서 자동 복원.
 *
 * <p>Cache 정책:
 * <ul>
 *   <li>Hit: Redis 에서 즉시 반환, TTL 갱신</li>
 *   <li>Miss + MySQL 메시지 있음: 직전 ASSISTANT 메시지의 references 로 복원 + Redis 재저장</li>
 *   <li>Miss + 메시지 없음: 빈 SessionData 반환 (새 세션)</li>
 * </ul>
 */
