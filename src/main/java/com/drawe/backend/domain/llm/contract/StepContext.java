 ...
 *
 * <p><strong>불변(record) + {@code @With}</strong>: Lombok 이
 * {@code withKeywords(...)}, {@code withReferences(...)} 등 wither 자동 생성.
 * 각 step 은 기존 ctx 를 수정하지 않고 새 ctx 를 반환.
 *
 * <h2>필드 분류</h2>
 * <ul>
 *   <li><strong>입력 (A가 채움, B는 읽기만)</strong>:
 *       {@code userId}, {@code projectId}, {@code sessionId},
 *       {@code rawMessage}, {@code cleanedMessage},
 *       {@code intent}, {@code uploadedImageUrl}, {@code previousReferences}</li>
 *   <li><strong>누적 (B가 채움)</strong>: {@code keywords}, {@code references}</li>
 *   <li><strong>누적 (A가 채움)</strong>: {@code generatedImage}, {@code composedAnswer}</li>
 * </ul>
 *
...
public record StepContext(
    ...
    /**
     * 새 파이프라인 시작 — A의 분류·전처리 결과로 초기 ctx 생성.
     * 누적 필드(keywords, references, generatedImage, composedAnswer) 는 null 로 시작.
     */
    public static StepContext start(
        ...
        return new StepContext(
                ...
                intent, uploadedImageUrl, previousReferences,
                null, null,    // keywords, references
                null, null     // generatedImage, composedAnswer
        );
    }
}
