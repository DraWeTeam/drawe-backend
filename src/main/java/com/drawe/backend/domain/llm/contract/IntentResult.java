 ...
 *
 * <p>개발자 A(결정·오케스트레이션) → 개발자 B(검색·데이터) 핸드오프 DTO.
 * 이 record의 필드는 양 트랙의 계약이므로 변경 시 양쪽 합의 필수.
 *
 * <p>S0(베타 중) 상태: 정의만 존재, 어디서도 사용하지 않음.
 * Phase 1(베타 후)에서 {@code IntentClassifier} 가 생성, {@code WorkflowService} 가 소비.
 *
 * <p>이전에 별도 IntentContext 를 둘지 검토했으나 이 record 의 슬롯이 충분하므로 합치기로 결정.
 *
 * @param code              분류된 의도 코드. 룰 매치 또는 경량 LLM 분류 결과.
 * @param referencedImages  사용자가 명시적으로 참조한 이미지 인덱스 목록 (예: "[2]번" → [2]).
 *                          1-based, 사용자 표시 순서 그대로. 이전 plan의 IntentCode 009를 슬롯으로 분리한 결과.
 *                          없으면 빈 리스트.
...
public record IntentResult(
   ...
   *
   * <p>LLM_MAIN 폴백은 SLO에서 사실상 발생하면 안 되는 케이스라 enum에서 제외했다.
   * 경량 LLM 분류가 실패하면 안전 기본값(SKIP 등)으로 폴백하는 것이 더 적절.
   */
  public enum Tier {
