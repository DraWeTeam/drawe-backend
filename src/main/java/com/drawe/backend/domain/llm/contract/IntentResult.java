 ...
 * 의도 분류 결과.
 *
 * <p>{@link IntentCode} + 슬롯({@code referencedImages}, {@code hasUploadedImage}) +
 * 메트릭 태그({@code tier}).
 *
 * <p>설계 결정:
 * <ul>
 *   <li>{@code Tier.LLM_MAIN} 없음 — 메인 LLM 분류 폴백은 SLO상 0이어야 하며,
 *       발생 시 안전 기본값({@link IntentCode#SKIP}) 처리.</li>
 *   <li>{@code referencedImages} 는 [N]번 앵커 슬롯 — IntentCode 009 (참조) 대체.</li>
 *   <li>{@code hasUploadedImage} 는 {@link IntentCode#SELF_CRITIQUE} (010) 트리거.</li>
 * </ul>
 *
...
public record IntentResult(
     ...
     *
     * <ul>
     *   <li>{@link #RULE} — 키워드 매처로 즉시 결정 (대다수)</li>
     *   <li>{@link #LLM_LIGHT} — 경량 LLM 분류 폴백 (룰 미스 시)</li>
     * </ul>
     */
    public enum Tier {
        RULE,
