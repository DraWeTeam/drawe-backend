...
public record ChatHistoryResponse(String sessionId, List<HistoryItem> messages) {
  ...
  public record HistoryItem(
    ...
    /**
     * 저장된 메시지를 응답으로 변환하면서 DB blob 이미지 URL({@code /images/{id}})에 서명을 붙인다. 채팅 히스토리를 다시 그릴 때도 AI 이미지가
     * 브라우저 {@code <img>} 로 로드돼야 하므로 노출 직전 서명이 필요하다. Unsplash 절대 URL 은 signer 가 그대로 통과시킨다.
     */
