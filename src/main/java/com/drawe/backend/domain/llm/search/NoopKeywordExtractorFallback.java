 ...
 *
 * <p>Grok 연결 전 임시. 호출 시 WARN 로그 + 빈 리스트 반환 → 검색이 안 되지만
 * 시스템 크래시는 안 함.
 *
 * <p>실제 LLM 폴백 추가 시:
 * <ol>
 *   <li>{@code GrokKeywordExtractorFallback implements KeywordExtractorFallback} 작성</li>
 *   <li>{@code @Component} 또는 {@code @Bean} 으로 등록</li>
 *   <li>이 클래스의 {@code @ConditionalOnMissingBean} 덕에 자동으로 비활성화</li>
 * </ol>
 *
...
public class NoopKeywordExtractorFallback {
    ...
    public KeywordExtractorFallback noopFallback() {
        log.warn("KeywordExtractorFallback 구현체가 없어 Noop 사용 — LLM 폴백 비활성. "
                + "실제 Grok 연결 시 GrokKeywordExtractorFallback 등록 필요.");
        return cleanedMessage -> {
            log.warn("Fallback called but not configured. Returning empty for: '{}'",
