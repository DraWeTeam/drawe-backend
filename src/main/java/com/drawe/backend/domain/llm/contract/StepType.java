 ...
 * 파이프라인 step 종류.
 *
 * <p>각 step 은 {@link StepExecutor} 구현체와 1:1 매핑. Spring {@code Map<StepType, StepExecutor>} 자동 주입 키로
 * 사용.
 *
 * <p>소유권:
 *
 ...
 * </table>
 *
 * <p>{@code TRANSLATE} 소유 (A) 결정 근거: 현재 PromptTranslator 는 단순 KO→EN 사전 매핑이 아니라 프로젝트 컨텍스트
 * (subject/technique/mood) 를 녹이고 Bria 이미지 생성용 prompt 로 정교하게 변환하는 Grok LLM 콜. B의 검색용 짧은 키워드 사전(3-6개)
 * 과는 목적·복잡도가 다름. 베타 후 B 사전이 풍부해지면 재검토.
 */
