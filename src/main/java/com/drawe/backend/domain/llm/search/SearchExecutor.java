...
public class SearchExecutor implements StepExecutor {
  ...
  public StepContext execute(StepContext ctx) {
    ...
    double avg = results.stream().mapToDouble(r -> scoreOf(r)).average().orElse(0.0);
    double max = results.stream().mapToDouble(r -> scoreOf(r)).max().orElse(0.0);
    double min = results.stream().mapToDouble(r -> scoreOf(r)).min().orElse(0.0);

    List<Long> imageIds = results.stream().map(ImageResult::id).toList();
    List<Double> scores = results.stream().map(r -> round3(scoreOf(r))).toList();
  ...
  /** null score 를 0.0 으로 간주 — 외부 결과에 null 이 섞여도 NPE 없이 처리. */
  private static double scoreOf(ImageResult r) {
    return r.score() == null ? 0.0 : r.score().doubleValue();
  }
