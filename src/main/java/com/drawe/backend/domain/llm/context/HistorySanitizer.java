 ...
 *
 * <p>S2' Phase 6 Layer 3. 옛 검색의 [1][2][3] 과 새 검색의 [1][2][3] 이 LLM 컨텍스트 안에서
 * 혼선되는 hallucination 차단.
 *
 * <p>현재 검색 결과를 가리키는 [N] 마커는 SYSTEM 블록 ([참고 이미지]) 에서만 유효하게 유지하고,
 * USER·ASSISTANT 메시지의 [N] 마커는 모두 제거한다. SYSTEM 메시지는 그대로 보존.
 *
 * <p>예:
 * <ul>
 *   <li>입력 ASSISTANT: "[1]번 이미지처럼 부드러운 색감을 표현해보세요"</li>
 *   <li>출력 ASSISTANT: "번 이미지처럼 부드러운 색감을 표현해보세요"</li>
 * </ul>
 *
 * <p>제거된 "번" 같은 부수 단어는 자연스러운 문장으로 남아 LLM 의 의미 이해엔 영향 적다.
 * 핵심은 옛 [N] 숫자가 새 [N] 인덱스와 충돌하지 않게 하는 것.
 */
@Component
