 ...
 *
 * <p>HTTP 클라이언트는 {@link UrlConnectionHttpClient} 를 명시한다 — 동기 S3Client 는 HTTP 구현체가 필요한데
 * {@code awssdk:s3} 가 자동으로 안 끌고 오기 때문(없으면 런타임에 Apache HC5 를 찾다 NoClassDefFoundError). PutObject +
